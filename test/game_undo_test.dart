...
void main() {
  test('undoMove should restore board state correctly in local multiplayer (single move)', () {
  ...
  test('undoMove should restore board state correctly in local multiplayer (two moves)', () {
    final container = ProviderContainer();
    final gameNotifier = container.read(gameProvider.notifier);
    ...
    expect(gameState.fen, equals(afterFirstMoveFen), reason: 'FEN should match e2-e4 state');
    expect(gameState.moveHistory.length, 1, reason: 'History should have 1 move');

    container.dispose();
  });

  test('undoMove should restore board state correctly in bot mode (undo both player and bot moves)', () {
    final container = ProviderContainer();
    final gameNotifier = container.read(gameProvider.notifier);
    ...
    gameState = container.read(gameProvider);
    expect(gameState.moveHistory.length, 2);
    expect(gameState.isPlayerTurn, true, reason: 'Should be player turn after bot move');
    ...
    expect(gameState.fen, equals(initialFen), reason: 'FEN should match initial FEN after undo (undo both moves)');
    expect(gameState.moveHistory.length, 0, reason: 'History should be empty after undo');

    container.dispose();
  });

  test('undoMove should restore board state correctly in bot mode (undo while bot thinking)', () {
    final container = ProviderContainer();
    final gameNotifier = container.read(gameProvider.notifier);
